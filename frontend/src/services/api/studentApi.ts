...
export const updatestudent=async(userId:string,updateData:Partial<IStudent>)=>{
  try {
    const response = await axiosInstance.put(`${API_URL}/student/update/${userId}`, {updateData}, { withCredentials: true });
      return response
  } catch (error) {
